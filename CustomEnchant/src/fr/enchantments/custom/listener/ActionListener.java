...
public class ActionListener implements Listener {
	...
	public void onEntityCreatePortalEvent(EntityCreatePortalEvent event) {
		if ((event.getEntity() instanceof EnderDragon && ExplosionHelper.getPortalLegality())) {
			event.setCancelled(true);
			ExplosionHelper.setPortalLegality();
		}
	...
	public void onEntityShootBowEvent(EntityShootBowEvent event) {
		...
		// if ( !EnchantmentHelper.haveCustomEnchant(projectileShooter) ) {
		// return; }

		// 4] And then do the cool things !
		Storage.ARROWOWNER.put(event.getProjectile().getUniqueId(),
				projectileShooter);

		// PluginLoader.pluginLoader.getServer().broadcastMessage(ChatColor.GREEN
		// + "HOOK UUID : " + event.getProjectile().getUniqueId());
	}
	...
	public void onProjectileHitEvent(ProjectileHitEvent event) {
		...
		if (event.getEntity().getType() == EntityType.SNOWBALL) {
			if (Storage.SNOWBALLOWNER.containsKey(event.getEntity()
					.getShooter())) {
				projectileShooter = Storage.SNOWBALLOWNER.get(event.getEntity()
						.getShooter());
				plugin.getFactory().projectileHitSomething(projectileShooter,
						event.getEntity());

				// Delay the suppression of the registration
				new RunnableDeadSnowBallRemover((Player) event.getEntity()
						.getShooter()).runTaskLater(plugin, 20);
				return;
			}
		}
	...
	public void onPlayerInteractEvent(PlayerInteractEvent event) {
		...
		if (event.getItem().getType() == Material.SNOW_BALL
			...
			Storage.SNOWBALLOWNER.put(event.getPlayer(), event.getPlayer()
					.getItemInHand());
		}
		if (
				(event.getAction() == Action.RIGHT_CLICK_AIR || event
			...
			plugin.getFactory().playerInteract(event.getPlayer(),event.getItem());
		}
	}
...
//		}
		...
		if (event instanceof EntityDamageByEntityEvent) {
			Entity tempEntityInflicter = ((EntityDamageByEntityEvent) event)
					.getDamager();
			if (tempEntityInflicter instanceof Projectile) {
				entityInflicter = ((Projectile) tempEntityInflicter)
						.getShooter();
		...
		if (entityInflicter != null) {
			eventId = entityVictim.getUniqueId().toString()
					+ entityInflicter.getUniqueId().toString();
		...
		// 2] Verification : does the fundamentals of physics are falling down ?
		if (entityVictim == null || entityInflicter == null) {
			return;
		}
		...
		if (event.getCause() == EntityDamageEvent.DamageCause.PROJECTILE) {
			if (projectile != null
				...
				if (Storage.SNOWBALLOWNER.containsKey(projectile.getShooter())) {
					weaponUsed = Storage.SNOWBALLOWNER.get(projectile
							.getShooter());
				}
			} else {
				if (!Storage.ARROWOWNER.containsKey(projectile.getUniqueId())) {
		...
		if (weaponUsed != null
				&& EnchantmentHelper.haveCustomEnchant(weaponUsed)) {
			plugin.getFactory().entityHit(entityInflicter, entityVictim,
					(int) event.getDamage());
		} else {
			ItemStack[] armorContents = entityVictim.getEquipment()
					.getArmorContents();
			for (ItemStack actualArmorPart : armorContents) {
				if (EnchantmentHelper.haveCustomEnchant(actualArmorPart)) {
					plugin.getFactory().entityHit(entityInflicter,
							entityVictim, (int) event.getDamage());
					return;
				}
			}
		}
